...
function UpdateGreatWorks()
	...
	if GwvSortOrder() then
		table.sort(allBuildings, function(a,b)
			...
			-- there are case that there can be palaces in multiple cities
			if a.buildingInfo.BuildingType == "BUILDING_PALACE" and b.buildingInfo.BuildingType ~= "BUILDING_PALACE" then
				return true;
			end
			if a.buildingInfo.BuildingType ~= "BUILDING_PALACE" and b.buildingInfo.BuildingType == "BUILDING_PALACE" then
				return false;
